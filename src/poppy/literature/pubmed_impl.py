...
def sleep_ncbi():    time.sleep(0.34)
def sleep_pubchem(): time.sleep(0.25)
def sleep_gbif():    time.sleep(0.20)  # be polite to GBIF
...
def maccs_from_smiles(smiles: str) -> str:
    if not (RDK_OK and smiles): return ""
    try:
...
SESSION.headers.update({
    "User-Agent": f"{TOOL} (mailto:{EMAIL})",
    ...
    for attempt in range(max_tries):
        try:
            ...
            try:
                ...
                # Fall back to XML
                try: return etree.fromstring(r.content)
                except Exception: pass
...
def esearch_count(query, mindate, maxdate):
    params = {"db":"pubmed","term":query,"datetype":"pdat","mindate":str(mindate),"maxdate":str(maxdate),
    ...
    if isinstance(resp, dict): return int(resp.get("esearchresult",{}).get("count",0))
    if isinstance(resp, etree._Element):
        cnt = resp.findtext(".//Count"); return int(cnt) if cnt and cnt.isdigit() else 0
    return 0

def esearch_ids_paged(query, mindate, maxdate, retstart=0, retmax=1000):
    params = {"db":"pubmed","term":query,"datetype":"pdat","mindate":str(mindate),"maxdate":str(maxdate),
              "retmode":"json","retstart":retstart,"retmax":retmax,"sort":"pub+date",
              "tool":TOOL,"email":EMAIL}
    resp = _safe_get(f"{EUTILS}/esearch.fcgi", params, sleep_fn=sleep_ncbi)
    if resp is None: return []
    if isinstance(resp, dict): return resp.get("esearchresult",{}).get("idlist",[]) or []
    if isinstance(resp, etree._Element): return [el.text for el in resp.findall(".//IdList/Id") if el is not None and el.text]
    return []

def _year_windows(start_year, end_year, span):
    windows = []; y = start_year
    while y <= end_year:
        y_end = min(y + span - 1, end_year)
        windows.append((y, y_end)); y = y_end + 1
    return windows
...
def esearch_all_ids_by_window(query, start_year, end_year, window_years=5, page_size=1000):
    ...
    for (y, y_end) in tqdm(windows, desc="Searching PubMed windows"):
        ...
        while retstart < count:
            ...
            if not ids: break
            pmids.update(ids); retstart += len(ids)
    return sorted(pmids)
...
def efetch_pubmed_xml(pmids):
    ...
    for i in tqdm(range(0, len(pmids), PMID_CHUNK),
                  total=total_batches, desc="Fetching+parsing articles"):
        sub = pmids[i:i+PMID_CHUNK]
        params = {"db":"pubmed","id":",".join(sub),"retmode":"xml","tool":TOOL,"email":EMAIL}
        root = _safe_get(f"{EUTILS}/efetch.fcgi", params, sleep_fn=sleep_ncbi, timeout=90)
        if not isinstance(root, etree._Element): continue
        for art in root.findall(".//PubmedArticle"):
            ...
            mesh_terms = [mh.findtext("DescriptorName") for mh in art.findall(".//MeshHeading") if mh.findtext("DescriptorName")]

            # ChemicalList (MeSH substances + RegistryNumber)
            chem_items = [{"name": (ch.findtext("NameOfSubstance") or "").strip(),
                           "rn":   (ch.findtext("RegistryNumber") or "").strip()}
                          for ch in art.findall(".//ChemicalList/Chemical")]
            # Supplementary Concepts (often chemicals)
            suppl = [(e.text or "").strip() for e in art.findall(".//SupplMeshNameList/SupplMeshName") if e.text]
            # Author Keywords
            keywords = [(k.text or "").strip() for k in art.findall(".//KeywordList/Keyword") if k.text]
            # DOI
            dois = [aid.text for aid in art.findall(".//ArticleIdList/ArticleId")
                    if aid.get("IdType") == "doi" and aid.text]
            doi  = (dois[0].lower().strip() if dois else "")

            yield {
                "pmid": pmid, "title": title, "abstract": abstract, "year": year,
                "journal": journal, "mesh": mesh_terms, "doi": doi,
                "chem_items": chem_items, "suppl": suppl, "keywords": keywords
            }

# ---------------- PLANT CONTEXT GATE ----------------
PLANT_MESH_HINTS = {"plants", "plants, medicinal", "plant extracts", "phytotherapy",
                    "pharmacognosy", "ethnopharmacology", "herbalism", "herbal medicine"}
PLANT_TA_HINTS   = re.compile(r"\b(plant|leaf|leaves|root|rhizome|seed|fruit|bark|flower|herb|herbal|botanical|"
...
GENUS_STOP = {
    "The","This","That","These","Those","A","An","In","On","Of","For","With","Without",
    "Although","While","Because","Since","If","No","Yes","Multiple","Spontaneous","Randomized",
    "Double","Open","Prospective","Retrospective","Comparative","Efficacy","Effects","Effect",
    "Intervention","Feeding","Findings","Study","Trial","Cohort","Participants","Patients",
    "Children","Adults","Women","Men","Case","Report","Review","Meta","Analysis","Method",
    "Methods","Materials","Results","Discussion","Conclusion","Conclusions","Retinal","Ganglion",
    "Cells","Cell","Brain","Salamander","Mouse","Mice","Rat","Rats","Human","Humans","Clinical",
    "Subthreshold","Threshold","Neural","Neuron","Cortex","Protein","Enzyme","Chromatography","Extraction"
}
...
def candidate_botanical_names(text):
    ...
    for m in BINOMIAL_FLEX.finditer(text):
        ...
        if infra:
            frag = m.group(0)
            if "var." in frag:   name += f" var. {infra}"
            elif "subsp." in frag or "ssp." in frag: name += f" subsp. {infra}"
            elif " f. " in frag or " forma " in frag: name += f" f. {infra}"
        cands.add(name)
    return sorted(cands)
...
def gbif_validate_plant(name):
    ...
    if not isinstance(resp, dict):
        GBIF_CACHE[name] = None; return None
...
INDICATION_SYNONYMS = {
    ...
    "antimicrobial": {"antimicrobial","antibacterial","antifungal","antiviral","antiparasitic","antimalarial","anthelmintic"},
    ...
    "gastroprotective": {"gastroprotective","ulcer","gastric","stomach ache","antidiarrheal","diarrhea","diarrhoea","constipation"},
    "hepatoprotective": {"hepatoprotective","liver","hepatitis"},
    "neuroprotective": {"neuroprotective","memory","neurodegenerative","anxiolytic","antidepressant","sedative"},
...
CHEM_HINTS = {
    "curcumin","demethoxycurcumin","bisdemethoxycurcumin","eugenol","rosmarinic acid",
    "caffeic acid","allicin","quercetin","kaempferol","luteolin","beta-sitosterol",
    "withaferin A","berberine","epigallocatechin gallate","EGCG","rutin","gallic acid",
    "ellagic acid","apigenin","naringenin","hesperidin","saponin","alkaloid","flavonoid","terpenoid"
}
COMPOUND_STOPWORDS = {
    "protein","chain","certain","margin","origin","skin","insulin",
    "stain","basin","main","grain","begin","kin","within","asin","login","admin"
}
...
def pubchem_cid_from_rn(rn):
    if not rn or not CAS_RE.match(rn): return None
    try:
        sleep_pubchem()
        ...
        r.raise_for_status()
        txt = r.text.strip()
        if txt: return int(txt.splitlines()[0])
    except Exception:
        return None
    return None
...
def pubchem_props(cid):
    ...
    resp = _safe_get(f"{PUBCHEM}/compound/cid/{cid}/property/{props}/JSON",
                     params={}, sleep_fn=sleep_pubchem, timeout=30)
...
# ---------------- RUN ----------------
def run():
    diag = {"pmids_total":0,"articles_seen":0,"plant_context_ok":0,
            "articles_with_species":0,"articles_with_compound_candidates":0,"kept_pairs":0}
    ...
    for a in efetch_pubmed_xml(pmids):
        ...
        for ch in a.get("chem_items", []):
            rn   = ch.get("rn","")
            name = ch.get("name","")
            if rn == "0": rn = ""  # PubMed uses "0" when no RN is assigned
            cand.add((rn, name))
