...
from .enrich import add_plants, add_compounds, add_genes, add_pathways, link_compound_to_plant
...
def build_from_config(cfg: Dict[str, Any]) -> Graph:
    ...
    # CSV files
    csv_files = (
        cfg.get("sources", {}).get("csv", {}).get("files", {})
        if cfg.get("sources", {}).get("csv", {}).get("enabled")
        else {}
    )
    ...
    if steps.get("link_compound_to_plant", False):
        if "cmap_plant_ingredient" in csv_files:
            ...
            link_compound_to_plant(
                g, ns, rel_rows, compound_id_key="compound_id", plant_id_key="plant_id"
            )
        else:
            comp_rows = get_rows_for("compounds")
            link_compound_to_plant(
                g, ns, comp_rows, compound_id_key="compound_id", plant_id_key="plant_id"
            )
