...
ingredient_target = pd.read_csv(
    "CMAUPv2.0_download_Ingredient_Target_Associations_ActivityValues_References.txt", sep="\t"
)

# Merge on Ingredient_ID
merged = pd.merge(
    step2, ingredient_target[["Ingredient_ID", "Target_ID"]], on="Ingredient_ID", how="left"
)
...
existing_smiles = {str(o).strip() for s, p, o in g.triples((None, hasSMILES, None))}
...
existing_links = {(str(s), str(o)) for s, p, o in g.triples((None, isDerivedFrom, None))}
...
with psycopg2.connect(
    host="unmtid-dbs.net", port=5433, dbname="drugcentral", user="drugman", password="dosage"
) as conn:
...
for idx, row in tqdm(
    action_type_df.iterrows(), total=len(action_type_df), desc="Enriching action_type"
):
    ikey = row["inchikey"]
...
# 6. Save result
g.serialize(
    destination="phytotherapies_final_enriched_with_ChEMBL_mechanisms_ATC_filtered.rdf",
    format="xml",
)
...
if not genetic_entity_uri:
    raise Exception("Could not find GeneticEntity class URI.")
...
print(
    f"Copied: {len(genetic_classes)} GeneticEntity-related classes, and {len(uri_map)} individuals (gene_ prefix removed and rdf:type preserved)."
)
